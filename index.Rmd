...
we divide the training test into pml_train_train and pml_train_test, and estimate the out of error using the pml_train_train data set. We try to predict using the pml_train_test and see if we are within the two standard deviations of the estimated out of error from pml_train_train data set.
...
the estimated out of sample error for CART is `r (1- cartFit$result[1,2])`
...
the estimated out of sample error for RandomForest is `r (1- rFfit$result[1,2])`

Since the estimated sample error for RandomForest is the smallest, we will use the RandomForest to predict the class of exercise
...
the overall accuracy on a new unseen sample data is `r confusionMatrix(predict(rFfit$finalModel, newdata=pml_train_test),pml_train_test$classe)$overall[1]` which is pretty good.
